...
static int set_bank(sensor_t *sensor, ov2640_bank_t bank)
{
    int res = 0;
    if (bank != reg_bank) {
        /* LIGHTGUN FIX: upstream updates the cache BEFORE the write is known to
         * have succeeded (here it is even set before the write is attempted). A
         * NACK on this 0xFF bank-select leaves the cache claiming a bank the
         * chip never switched to — a retry of the whole register sequence then
         * skips the bank select ("already there") and writes DSP values into
         * SENSOR registers. Those writes ACK, so the retry wrapper reports
         * SUCCESS on a scrambled sensor. Only cache the bank once ACKed. */
        res = SCCB_Write(sensor->slv_addr, BANK_SEL, bank);
        if (res == 0) {
            reg_bank = bank;
        } else {
            reg_bank = BANK_MAX;      /* unknown: force a re-select next time */
        }
    }
    return res;
}
...
static int read_reg(sensor_t *sensor, ov2640_bank_t bank, uint8_t reg)
{
    if(set_bank(sensor, bank)){
        return -1;      /* LIGHTGUN: was 0 — a failed bank select reported the
                         * register as 0x00, which set_reg then wrote back. */
    }
    extern volatile bool sccb_read_failed;
    int v = SCCB_Read(sensor->slv_addr, reg);
    /* LIGHTGUN: distinguish a real 0xFF from a failed read (see sccb.c). This
     * makes the `if (ret < 0)` guards live, so a read-modify-write can no
     * longer compute its new value from all-ones. */
    if (sccb_read_failed) return -1;
    return v;
}
